...
function ThreadIcon({ iconKey }: { iconKey: MessageThreadIconKey }) {
  if (iconKey === "bell") return <svg aria-hidden="true" viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M12 4a4 4 0 0 0-4 4v1.3c0 .8-.2 1.5-.6 2.2L6 14h12l-1.4-2.5c-.4-.7-.6-1.4-.6-2.2V8a4 4 0 0 0-4-4Z" /><path d="M10 18a2 2 0 0 0 4 0" /></svg>;
          ...
          {posts.length > 0 ? posts.map((post) => (
            <div key={post.id} style={{ maxWidth: "min(100%, 720px)", padding: "14px 16px", borderRadius: 14, backgroundColor: "rgba(15, 23, 42, 0.72)", border: "1px solid rgba(148, 163, 184, 0.14)" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: post.imageUrl ? "120px minmax(0, 1fr)" : "minmax(0, 1fr)", gap: 14, alignItems: "start" }}>
                {post.imageUrl ? <img src={post.imageUrl} alt={post.title} style={{ width: 120, height: 120, objectFit: "cover", borderRadius: 12, border: "1px solid rgba(148, 163, 184, 0.14)" }} /> : null}
                <div>
